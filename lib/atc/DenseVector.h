...
public:
  ...
  DenseVector(const DenseVector<T> &c) : _data(nullptr) { _copy(c); } 
  DenseVector(const Vector<T> &c)      : _data(nullptr) { _copy(c); } 
  DenseVector(const T * ptr, INDEX nrows) : _data(nullptr) { copy(ptr,nrows); } 
